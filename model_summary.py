...
def get_model_flops(model, input_res,lms,print_per_layer_stat=True,
...
def get_model_activation(model, input_res, input_constructor=None):
    ...
    if input_constructor:
        ...
        batch,nouse = torch.FloatTensor(1, *input_res).to(device)
...
def flops_to_string(flops, units='GMac', precision=2):
    if units is None:
        if flops // 10**9 > 0:
            return str(round(flops / 10.**9, precision)) + ' GMCNet'
        elif flops // 10**6 > 0:
